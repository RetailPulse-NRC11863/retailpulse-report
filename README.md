...
| TB2 | 06/07/2026 | Equipo RetailPulse (Fabio Vallejo, Jesus Godoy, Luis Rubio, Anghelo Hurtado, José del Carpio) | Complemento del Capítulo V para la entrega final, incorporando evidencias de flujos completos de negocio, trazabilidad entre frontend, backend y reglas de dominio, actualización de despliegue en Azure Static Web Apps y Azure Web Apps for Containers, y planificación del Sprint 4 con las User Stories pendientes del Product Backlog. |
...
Como complemento final del Sprint 3, se revisó la documentación para retirar la idea de módulos aislados y presentar RetailPulse como un sistema de flujos conectados. La versión final evidencia que una acción del comprador en kiosko puede modificar métricas de conversión, generar solicitudes de ayuda o derivar en tareas visibles para el staff; asimismo, una actualización de inventario desde administración puede activar reglas de alerta operativa y alimentar recomendaciones comerciales basadas en productos reales de la tienda.
...
| Endpoints consumidos | `GET /api/v1/kiosk/products/search?query={texto}`, `POST /api/v1/kiosk/sessions/{sessionId}/searches` |
| Entidades involucradas | Product, Zone, InventoryItem, KioskSession, ProductSearch |
| Resultado esperado | El comprador visualiza productos coincidentes con stock, referencia de estante y un layout completo de tienda donde se resalta la zona del producto. |
...
<img src="assets/images/sprint-3-kiosk-layout-full-store.png" alt="Layout completo de tienda desplegado desde el kiosko con zona del producto resaltada" width="800">
...
**6. Flujo de inventario bajo que genera alerta y tarea operativa:**

Este flujo evidencia una regla de negocio central para RetailPulse: el sistema no solo permite modificar stock, sino que interpreta el cambio y genera trabajo operativo cuando el inventario cae por debajo del umbral configurado. De esta forma, la vista administrativa, el inventario y la vista de staff quedan conectados dentro de un proceso de negocio observable.

| Elemento | Evidencia |
| :--- | :--- |
| Pantallas frontend | `/app/admin/store-configuration`, `/app/staff/alerts` |
| Servicios Angular | `StoreConfigurationStoreService`, `StoreOperationsStore`, `OperationalTasksApiService` |
| Endpoints relacionados | `PATCH /api/v1/inventory/items/{productId}/stock`, `GET /api/v1/operational-alerts/active`, `GET /api/v1/operational-tasks/pending` |
| Entidades involucradas | Product, InventoryItem, OperationalAlert, OperationalTask |
| Estados relevantes | `AVAILABLE` -> `LOW_STOCK` o `OUT_OF_STOCK`; `PENDING` -> `COMPLETED` |
| Regla de negocio | Si el stock actualizado es menor o igual al umbral crítico, el sistema crea una alerta y una tarea de reposición asociadas al producto y zona correspondiente. |
| Resultado esperado | El administrador actualiza inventario y el staff recibe una tarea accionable sin depender de carga manual. |

<img src="assets/images/sprint-3-low-stock-alert-task.png" alt="Inventario bajo generando alerta y tarea operativa para staff" width="800">

**7. Flujo de recomendación comercial aplicada:**
...
| Endpoints relacionados | `GET /api/v1/promotion-recommendations/product-opportunities`, `GET /api/v1/promotion-recommendations/active`, `PATCH /api/v1/promotion-recommendations/{recommendationId}/apply` |
| Entidades involucradas | PromotionRecommendation, Product, ProductSearch, InventoryItem |
| Estados relevantes | `ACTIVE` -> `APPLIED` |
| Regla de negocio | Los productos con interacción de kiosko y conversión baja, o con riesgo de inventario, se clasifican como oportunidades comerciales y pueden generar recomendaciones de descuento, reubicación, asistencia o reposición. |
| Resultado esperado | El administrador visualiza recomendaciones basadas en productos reales, aplica una acción comercial y el sistema actualiza el estado de la recomendación. |
...
* Documentación de endpoints REST orientados a integración progresiva entre frontend, backend y base de datos.
* Documentación de requests, responses y endpoints críticos para evidenciar consumo real desde frontend y Swagger/OpenAPI.
...
| Store Foundation | `/api/v1/stores` | GET, POST | Consulta y creación de tiendas asociadas a cuentas SaaS | Implementado y documentado |
| Store Foundation | `/api/v1/stores/{storeId}` | GET, PUT | Consulta y actualización de información de tienda | Implementado y documentado |
| Store Foundation | `/api/v1/zones` | GET, POST | Consulta y creación de zonas del layout físico de tienda | Implementado y documentado |
| Store Foundation | `/api/v1/zones/{zoneId}` | GET, PUT, DELETE | Consulta, edición y eliminación de zonas del layout | Implementado y documentado |
| Store Foundation | `/api/v1/products` | GET, POST | Consulta y creación de productos reales de la tienda | Implementado y documentado |
| Store Foundation | `/api/v1/products/search` | GET | Búsqueda administrativa de productos por nombre o SKU | Implementado y documentado |
| Store Foundation | `/api/v1/products/{productId}` | GET, PUT, DELETE | Consulta, actualización y eliminación de productos | Implementado y documentado |
...
| Traffic Analytics | `/api/v1/traffic/zones/{zoneId}/metrics` | POST, PUT | Registro o actualización de métricas de tráfico, permanencia, interacción y conversión por zona | Implementado y documentado |
...
| Assisted Shopping / Kiosk | `/api/v1/kiosk/sessions/{sessionId}/searches` | POST | Registro de acciones de búsqueda: `SEARCHED`, `LOCATION_VIEWED`, `HELP_REQUESTED`, `FOUND` | Implementado y documentado |
...
| Store Operations | `/api/v1/operational-alerts/{alertId}/resolve` | PATCH | Resolución de una alerta operativa | Implementado y documentado |
...
| Store Operations | `/api/v1/operational-tasks/{taskId}/complete` | PATCH | Cambio de estado de una tarea de `PENDING` a `COMPLETED` | Implementado y documentado |
| Promotion Optimization | `/api/v1/promotion-recommendations` | GET, POST | Consulta y creación de recomendaciones comerciales | Implementado y documentado |
| Promotion Optimization | `/api/v1/promotion-recommendations/active` | GET | Consulta de recomendaciones activas | Implementado y documentado |
| Promotion Optimization | `/api/v1/promotion-recommendations/{recommendationId}/apply` | PATCH | Aplicación de recomendación comercial y cambio de estado | Implementado y documentado |
| Promotion Optimization | `/api/v1/promotion-recommendations/product-opportunities` | GET | Consulta de oportunidades comerciales calculadas desde productos reales, inventario e interacciones de kiosko | Implementado y documentado |
| Subscription | `/api/v1/subscription/plans` | GET | Consulta de planes SaaS disponibles | Implementado y documentado |
...
* **Enlace de Producción (Backend): https://retailpulse-api-fabio-dgb7etencqega8g8.centralus-01.azurewebsites.net/swagger-ui/index.html**
...
| Backend Spring Boot | Azure Web Apps | Acceso público a Swagger/OpenAPI y endpoints REST bajo `/api/v1`. |
...
El Sprint 4 se plantea como el sprint de consolidación final de RetailPulse. A diferencia de los sprints anteriores, su propósito no es agregar operaciones CRUD aisladas, sino cerrar las brechas identificadas durante la retroalimentación docente y demostrar que la plataforma implementa procesos de negocio completos, con entidades que atraviesan estados y reglas explícitas.

El foco del sprint se orienta a conectar los módulos ya existentes: kiosko, catálogo, inventario, métricas de tráfico, recomendaciones, alertas y tareas de staff. De esta manera, las acciones realizadas por un comprador, administrador o colaborador de tienda dejan evidencia en otras partes del sistema y permiten sustentar el valor de RetailPulse frente a soluciones tradicionales de búsqueda de productos o dashboards estáticos.
...
<table>
  ...
  <tr>
    <td><strong>Sprint Planning Background</strong></td>
    <td>
      Cuarta planificación del proyecto enfocada en la consolidación final del sistema, la corrección de brechas detectadas durante la revisión parcial y la preparación de evidencias de flujos completos de negocio. El equipo priorizó funcionalidades donde una acción del usuario produce consecuencias trazables en otros módulos, evitando que el sistema se perciba como una colección de pantallas o CRUDs independientes.
    </td>
  </tr>
  ...
  <tr>
    <td>Location</td>
    <td>Modalidad remota mediante Discord y coordinación por GitHub</td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Durante el Sprint 3 se implementó la primera versión real del backend con Spring Boot y se integraron endpoints principales con la aplicación Angular. Sin embargo, a partir del feedback recibido se identificó que el sistema debía evidenciar con mayor claridad procesos de negocio completos, reglas de negocio, cambios de estado y trazabilidad entre los roles de administrador, staff y comprador.
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      El equipo concluyó que algunas vistas todavía podían percibirse como módulos independientes. Por ello, el Sprint 4 priorizó la conexión entre productos reales, búsqueda en kiosko, métricas de interacción, inventario, alertas, tareas y recomendaciones comerciales, además de mejorar la documentación y la evidencia técnica para la entrega final.
    </td>
  </tr>
  ...
  <tr>
    <td>Sprint 4 Goal</td>
    <td>
      El objetivo del Sprint 4 es consolidar RetailPulse como una plataforma orientada a flujos de negocio completos para tiendas físicas. Para ello, se busca que los eventos generados por compradores, administradores y staff activen reglas de negocio visibles: congestión crítica, alertas por inventario, tareas operativas, recomendaciones comerciales y promociones dinámicas vinculadas a productos reales.
      <br><br>
      Asimismo, el sprint contempla estabilizar el despliegue final en Azure, preparar seed data representativa, asegurar separación por tienda/cuenta y dejar evidencias documentables para demostrar que la solución supera una implementación basada únicamente en CRUDs.
    </td>
  </tr>
...
Durante el Sprint 4, la colaboración del equipo se concentró en la estabilización final de RetailPulse como sistema integrado. Las responsabilidades se organizaron alrededor de flujos completos de negocio, despliegue final y documentación de evidencias. Esta distribución permitió conectar el trabajo de frontend, backend, base de datos y reporte bajo un mismo objetivo: demostrar reglas de negocio observables y trazabilidad entre roles.

| Team Member (Last Name, First Name) | GitHub Username | Backend v2 & Azure Deployment | Kiosk Business Flow | Store Operations Rules | Conversion & Recommendations | Store Context & Data Isolation | Sprint 4 Report Evidence |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| Vallejo Trujillo, Fabio Cesar | fabiovallejo | L | L | C | C | L | L |
| Faustino Hurtado, Anghelo Edwin | Limos05 | C | C | L | C | C | C |
| Franco del Carpio, José María | VoltTrd | C | C | C | C | L | C |
| Godoy Santillan, Jesus Andres | JesusGodoyS | C | L | C | C | C | C |
| Rubio Ortiz, Luis Sebastián | notoriussxd | C | C | C | L | C | C |

##### 5.2.4.3. Sprint Backlog 4

![Sprint Backlog 4](assets/images/sprint_backlog_4.png)

**Duración:** 2 semanas

**Objetivo del Sprint:**
Conectar los módulos principales de RetailPulse mediante reglas de negocio y cambios de estado observables, reforzando la trazabilidad entre acciones del kiosko, inventario, métricas de tienda, recomendaciones comerciales, alertas y tareas del staff.
...
<tr>
  ...
  <td rowspan="2">Recibir alertas de congestión operativa</td>
  <td>T-35</td>
  <td>Regla de alerta por congestión</td>
  <td>Relacionar métricas de tráfico e intensidad por zona con la creación de alertas operativas para el staff cuando una zona supera niveles críticos.</td>
  <td>5</td>
  <td>Vallejo Trujillo, Fabio Cesar</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-36</td>
  <td>Evidencia de alerta en staff</td>
  <td>Validar que una alerta generada por condición operativa sea visible en la vista del personal de tienda y pueda ser atendida.</td>
  <td>4</td>
  <td>Faustino Hurtado, Anghelo Edwin</td>
  <td>Done</td>
</tr>
...
<tr>
  <td rowspan="2">US-08</td>
  <td rowspan="2">Detectar patrones de circulación</td>
  <td>T-37</td>
  <td>Layout visual reutilizable</td>
  <td>Utilizar el layout configurado por el administrador para visualizar zonas de tienda tanto en el heatmap como en la ubicación de productos desde kiosko.</td>
  <td>6</td>
  <td>Rubio Ortiz, Luis Sebastián</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-38</td>
  <td>Métricas distribuidas por zona</td>
  <td>Preparar datos de tráfico, permanencia, intensidad y conversión por zona para evidenciar diferencias visuales en el heatmap.</td>
  <td>5</td>
  <td>Vallejo Trujillo, Fabio Cesar</td>
  <td>Done</td>
</tr>
...
<tr>
  ...
  <td rowspan="2">Activar alerta por quiebre de stock consultado</td>
  <td>T-39</td>
  <td>Solicitud de ayuda desde kiosko</td>
  <td>Registrar la acción `HELP_REQUESTED` cuando el comprador requiere asistencia y conectarla con la creación de una tarea operativa.</td>
  <td>5</td>
  <td>Godoy Santillan, Jesus Andres</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-40</td>
  <td>Alerta por stock bajo o agotado</td>
  <td>Generar alerta y tarea cuando el stock actualizado cae por debajo del umbral crítico o llega a cero unidades.</td>
  <td>5</td>
  <td>Faustino Hurtado, Anghelo Edwin</td>
  <td>Done</td>
</tr>
...
<tr>
  ...
  <td rowspan="2">Acceder a información según rol operativo</td>
  <td>T-41</td>
  <td>Identificación visible de cuenta</td>
  <td>Mostrar el usuario activo en vistas administrativas y de staff para reducir ambigüedad durante pruebas y exposición.</td>
  <td>3</td>
  <td>Franco del Carpio, José María</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-42</td>
  <td>Separación de datos por tienda</td>
  <td>Aplicar contexto de owner/store para que una cuenta nueva no herede productos, zonas, métricas o alertas de la tienda demo.</td>
  <td>5</td>
  <td>Vallejo Trujillo, Fabio Cesar</td>
  <td>Done</td>
</tr>
...
<tr>
  ...
  <td rowspan="2">Detectar automáticamente congestión crítica</td>
  <td>T-43</td>
  <td>Umbrales de congestión</td>
  <td>Definir criterios de intensidad, tráfico y estado de congestión para distinguir zonas frías, moderadas y críticas.</td>
  <td>4</td>
  <td>Rubio Ortiz, Luis Sebastián</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-44</td>
  <td>Ciclo de vida de alerta crítica</td>
  <td>Documentar y validar el flujo desde detección de congestión hasta visualización y atención por parte del staff.</td>
  <td>4</td>
  <td>Faustino Hurtado, Anghelo Edwin</td>
  <td>Done</td>
</tr>
...
<tr>
  ...
  <td rowspan="2">Recibir promociones dinámicas según comportamiento</td>
  <td>T-45</td>
  <td>Promociones conectadas a productos reales</td>
  <td>Vincular recomendaciones y promociones del kiosko con productos reales de la tienda, evitando datos comerciales hardcodeados.</td>
  <td>5</td>
  <td>Rubio Ortiz, Luis Sebastián</td>
  <td>Done</td>
</tr>
...
<tr>
  <td>T-46</td>
  <td>Conversión basada en interacción</td>
  <td>Calcular oportunidades comerciales usando interacciones de kiosko, acciones `FOUND`, inventario y recomendaciones aplicables.</td>
  <td>5</td>
  <td>Vallejo Trujillo, Fabio Cesar</td>
  <td>Done</td>
</tr>
...
Durante el Sprint 4, el desarrollo se orientó a consolidar la versión final de RetailPulse y corregir las brechas detectadas durante la revisión parcial. El trabajo técnico se enfocó en conectar flujos de negocio completos, actualizar el backend a una versión desplegable mediante Docker, asegurar que el frontend consuma datos reales por tienda y fortalecer la evidencia de reglas de negocio.

| Repository | Branch | Commit Message | Author |
| :--- | :--- | :--- | :--- |
| retailpulse-web-services | main / develop | feat: support business flow data and store-scoped operations | fabiovallejo |
| retailpulse-web-application | main / develop | feat: connect retail flows and improve kiosk/store UI | fabiovallejo |
| retailpulse-report | develop | docs: document sprint 4 final business flows and deployment evidence | fabiovallejo |

La evidencia de desarrollo del Sprint 4 demuestra la transición desde una integración básica de servicios hacia una solución donde las entidades centrales atraviesan estados controlados por reglas de negocio. Las principales capacidades implementadas fueron:

| Capacidad desarrollada | Módulos relacionados | Resultado técnico |
| :--- | :--- | :--- |
| Registro de acciones de kiosko | Assisted Shopping, ProductSearch | Las acciones `SEARCHED`, `LOCATION_VIEWED`, `HELP_REQUESTED` y `FOUND` quedan registradas como eventos trazables. |
| Tareas operativas por reglas | Inventory Intelligence, Store Operations | El stock bajo, el quiebre de stock y la solicitud de ayuda generan alertas y tareas para staff. |
| Conversión basada en comportamiento | Assisted Shopping, Promotion Optimization | La acción `FOUND` alimenta métricas de conversión y oportunidades comerciales sobre productos reales. |
| Layout reutilizado entre módulos | Store Foundation, Traffic Analytics, Kiosk | El layout configurado por administración se reutiliza en heatmap y en búsqueda visual desde kiosko. |
| Separación por tienda/cuenta | Subscription, Store Foundation, HTTP Interceptor | Las consultas se filtran por contexto de usuario para evitar que una cuenta nueva herede datos de la tienda demo. |
| Backend v2 desplegable | Docker, Azure Web Apps, PostgreSQL | La API se empaqueta como imagen Docker y se despliega en Azure conectada a una base PostgreSQL limpia. |

##### 5.2.4.5. Execution Evidence for Sprint Review

La ejecución del Sprint 4 se validó mediante flujos de negocio completos. Cada flujo parte de una acción realizada por un usuario y termina en un cambio observable dentro del sistema, como una actualización de estado, una nueva tarea operativa, una métrica actualizada o una recomendación comercial.

**1. Flujo de búsqueda visual de producto desde kiosko**

El comprador accede al modo kiosko, busca un producto, consulta su stock y despliega el layout completo de la tienda. El sistema resalta la zona correspondiente al producto, registra la acción `LOCATION_VIEWED` y mantiene la ubicación dentro del contexto visual del local.

| Elemento | Evidencia |
| :--- | :--- |
| Pantalla frontend | `/kiosk` |
| Endpoints consumidos | `GET /api/v1/kiosk/products/search`, `POST /api/v1/kiosk/sessions/{sessionId}/searches` |
| Entidades involucradas | Product, Zone, InventoryItem, KioskSession, ProductSearch |
| Regla de negocio | La visualización de ubicación registra interacción del comprador con el producto. |
| Resultado | El cliente identifica el producto dentro del layout real de tienda y el sistema registra la interacción. |

<img src="assets/images/sprint-4-kiosk-full-layout-location.png" alt="Kiosko mostrando layout completo con zona del producto resaltada" width="800">

**2. Flujo de solicitud de ayuda y generación de tarea para staff**

Cuando el comprador solicita ayuda desde el kiosko, el sistema registra la acción `HELP_REQUESTED` y crea una tarea operativa asociada al producto, tienda y zona. La tarea aparece en la vista del staff para ser atendida.

| Elemento | Evidencia |
| :--- | :--- |
| Pantallas frontend | `/kiosk`, `/app/staff/alerts` |
| Endpoints consumidos | `POST /api/v1/kiosk/sessions/{sessionId}/searches`, `GET /api/v1/operational-tasks/pending` |
| Entidades involucradas | ProductSearch, OperationalTask, OperationalAlert |
| Cambio de estado | Nueva tarea en estado `PENDING` hasta su atención por staff. |
| Resultado | La necesidad del comprador se convierte en trabajo operativo visible para el personal de tienda. |

<img src="assets/images/sprint-4-help-requested-task.png" alt="Solicitud de ayuda desde kiosko generando tarea para staff" width="800">

**3. Flujo de inventario bajo y tarea de reposición**

Cuando el administrador actualiza el stock de un producto y este queda bajo el umbral configurado, el sistema clasifica el inventario como `LOW_STOCK` u `OUT_OF_STOCK` y genera una alerta operativa junto con una tarea de reposición.

| Elemento | Evidencia |
| :--- | :--- |
| Pantallas frontend | `/app/admin/store-configuration`, `/app/staff/alerts` |
| Endpoints consumidos | `PATCH /api/v1/inventory/items/{productId}/stock`, `GET /api/v1/operational-alerts/active`, `GET /api/v1/operational-tasks/pending` |
| Entidades involucradas | InventoryItem, OperationalAlert, OperationalTask |
| Cambio de estado | `AVAILABLE` -> `LOW_STOCK` u `OUT_OF_STOCK`; tarea `PENDING` -> `COMPLETED`. |
| Resultado | La actualización de inventario produce una acción operativa concreta para el staff. |

<img src="assets/images/sprint-4-low-stock-rule.png" alt="Actualización de stock generando alerta y tarea operativa" width="800">

**4. Flujo de recomendación comercial aplicada**

El administrador revisa oportunidades comerciales basadas en productos reales, interacciones del kiosko, conversión e inventario. Al aplicar una recomendación, el sistema cambia su estado de `ACTIVE` a `APPLIED`.

| Elemento | Evidencia |
| :--- | :--- |
| Pantalla frontend | `/app/admin/conversion` |
| Endpoints consumidos | `GET /api/v1/promotion-recommendations/product-opportunities`, `PATCH /api/v1/promotion-recommendations/{recommendationId}/apply` |
| Entidades involucradas | Product, ProductSearch, InventoryItem, PromotionRecommendation |
| Cambio de estado | `ACTIVE` -> `APPLIED`. |
| Resultado | La recomendación deja de ser solo informativa y se convierte en una decisión comercial registrada. |

<img src="assets/images/sprint-4-real-product-recommendations.png" alt="Recomendaciones comerciales basadas en productos reales" width="800">

##### 5.2.4.6. Services Documentation Evidence for Sprint Review.

En el Sprint 4 se documentó la versión v2 del backend de RetailPulse, enfocada en endpoints que sostienen flujos completos de negocio. La API se mantuvo bajo la ruta base `/api/v1`, pero se reforzó la relación entre recursos, reglas y estados para demostrar que el sistema no depende de actualizaciones manuales aisladas.

| Flujo soportado | Endpoints principales | Request / Response documentado | Estado |
| :--- | :--- | :--- | :--- |
| Búsqueda y ubicación visual en kiosko | `GET /api/v1/kiosk/products/search`, `GET /api/v1/kiosk/products/{productId}` | Respuesta con producto, inventario, zona y layout de tienda | Documentado |
| Registro de acciones del comprador | `POST /api/v1/kiosk/sessions/{sessionId}/searches` | Request con `query`, `productId`, `resultStatus` y `action` | Documentado |
| Consulta de layout y métricas por zona | `GET /api/v1/zones`, `GET /api/v1/traffic/zones/metrics`, `PUT /api/v1/traffic/zones/{zoneId}/metrics` | Respuesta con tráfico, permanencia, intensidad y conversión | Documentado |
| Actualización de inventario | `PATCH /api/v1/inventory/items/{productId}/stock` | Request con stock disponible y respuesta con estado derivado | Documentado |
| Alertas y tareas operativas | `GET /api/v1/operational-alerts/active`, `GET /api/v1/operational-tasks/pending`, `PATCH /api/v1/operational-tasks/{taskId}/complete` | Respuestas con prioridad, estado, producto, zona y origen de la regla | Documentado |
...
* **Plataforma:** Azure Static Web Apps
* **URL:** https://polite-sea-0e075210f.7.azurestaticapps.net/

<img src="assets/images/sprint-4-azure-static-web-app.png" alt="Frontend final desplegado en Azure Static Web Apps" width="800">

**2. Despliegue del Backend v2**

* **Plataforma:** Azure App Service
* **Imagen:** `retailpulse-backend:2.0.0`
* **Runtime:** Java 21 sobre contenedor Docker
* **URL Swagger:** https://retailpulse-api-fabio-dgb7etencqega8g8.centralus-01.azurewebsites.net/swagger-ui/index.html

<img src="assets/images/sprint-4-azure-container-v2.png" alt="Backend v2 ejecutándose como contenedor en Azure Web Apps" width="800">

**3. Verificación de health check**

La ruta `/api/v1/health` confirmó la disponibilidad del backend y las vistas del frontend validaron consumo hacia `environment.apiUrl`, configurado con la URL pública del backend.

<img src="assets/images/sprint-4-health-check.png" alt="Health check exitoso del backend v2 en Azure" width="800">

##### 5.2.4.8. Team Collaboration Insights during Sprint.

Durante el Sprint 4, el equipo trabajó sobre una dinámica de cierre orientada a estabilización, integración y documentación final. La coordinación se concentró en revisar el feedback recibido, convertirlo en mejoras funcionales y evidenciar resultados mediante flujos de negocio completos.

**Desarrollo de las Actividades de Implementación**

* Se consolidó la versión final del backend con reglas de negocio conectadas entre kiosko, inventario, operaciones y recomendaciones.
* Se estabilizó el frontend para mostrar contexto de usuario, layout visual completo en kiosko, datos reales por tienda y consumo de endpoints finales.
* Se corrigió la configuración de despliegue para Azure, incluyendo imagen Docker, variables de entorno, CORS, perfiles de ejecución y seed data.
* Se reforzó el reporte para documentar Sprint 4 con la misma estructura del Sprint 3 y con evidencias orientadas a procesos de negocio.

**Evidencia de Colaboración en GitHub**

<img src="assets/images/team-collaboration-insights-sprint-4.png" alt="GitHub Insights Sprint 4" width="800">

**Insights del Sprint 4:**

* El trabajo se organizó alrededor de flujos funcionales completos, no por pantallas aisladas.
* La integración final requirió coordinación entre frontend, backend, base de datos, despliegue y documentación.
* Las actividades de cierre permitieron transformar feedback académico en mejoras visibles del sistema.
* La revisión final fortaleció la comunicación técnica del proyecto al vincular reglas de negocio, estados y resultados observables.
...
En esta sección se presenta la evaluación heurística aplicada al landing page y a la aplicación web de RetailPulse, considerando los criterios definidos para el proyecto.
...
[https://upcedupe-my.sharepoint.com/:v:/g/personal/u202310349_upc_edu_pe/IQBrOnJJU_MkTL4CGkC0d8lIAc8mXpNImVidVC8DSX7uJj8?e=Rl3OJ8](https://upcedupe-my.sharepoint.com/:v:/g/personal/u202310349_upc_edu_pe/IQBrOnJJU_MkTL4CGkC0d8lIAc8mXpNImVidVC8DSX7uJj8?e=Rl3OJ8)

**URL de la versión publicada en YouTube:**

[https://youtu.be/IugIQcvcHa4](https://youtu.be/IugIQcvcHa4)
...
https://retailpulse-api-fabio-dgb7etencqega8g8.centralus-01.azurewebsites.net/swagger-ui/index.html
